...
function Map() {
  ...
  const refreshButton = () => {
    ...
    return (
      <button onClick={refreshActivities} className="shadow-md absolute mt-4 mr-8 z-500 top-0 right-0 whitespace-nowrap inline-flex items-center justify-center px-2 py-1 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-white hover:bg-gray-100 text-purple-600 font-sans">
    ...
    return(
      <div className="absolute ml-4 mb-8 z-500 bottom-0 left-0 flex" id="filters">
        ...
        <MultiSelect
          ...
          className="flex items-center bg-white rounded-md ml-2 shadow-md relative"
          triggerContent={<span>Type{typesDisplayLabel()}</span>}
          options={options}
          selectedOptions={(filters.activity_types || []).map((label) => {
            return options.find((opt) => { return opt.label === label})
          })}
        />
